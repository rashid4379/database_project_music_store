...
if (isset($_POST['submit'])) {
   ...
   // $as_id=$_POST['as_id'];
    ...
    //$as_upload_date=$_POST['as_upload_date'];
    ...
    //echo "<script>alert('$album_description')</script>";
      ...
      $get_cats="select * from singers where singer_name='$singer_name'";
                ...
                // get the album name here
               $get_cats="select * from album where album_name='$album_name'";
                
                $run_cats=mysqli_query($connection, $get_cats);
                
                while ($row_cats=mysqli_fetch_array($run_cats)){
                  
                  $album_id=$row_cats['album_id'];

               }

    $insert = "INSERT INTO audio_songs (singer_id,album_id,as_name,as_upload_date)values($singer_id,$album_id,'$as_name',now())";
    $query= mysqli_query ($connection,$insert);

    if($query)
    {
        echo "<script>alert('record has been entered')</script>";
        echo "<script>window.openalert('add_audio.php','_self')</script>";
    }
    else
    {
        ...
        <ul style="background-color: black;" >
            ...
            <li style="padding-left: 60px;">
             ...
             <ul>
               ...
               <div class="container">
  <h2 style="color: white">Add Aduio Songs</h2>

  <!-- form start here -->
  <form class="form-horizontal" method='post' action='add_audio.php'  enctype='multipart/form-data' >
    ...
    <div class="form-group">
      <label class="control-label col-sm-2" for="email" style="color: white">Audio Song Name:</label>
      <div class="col-sm-4">
        <input type="text" class="form-control" name="as_name" placeholder="Enter audio Song name">
      </div>
    </div>
      <!--<div class="form-group">
    ...
    <div class='form-group'>
              ...
              ?>
                         <!-- php code for view category data end here-->
                                   ...
                                   <!--- foreign key album of a adding -->
    <div class='form-group'>
                               <label class="control-label col-sm-2" for="pwd" style="color: white">Select album</label>
                                    <div class='col-sm-10'>
              ...
              <?php 
                ...
                $get_cats="select distinct album_name from album";
                
                $run_cats=mysqli_query($connection, $get_cats);
                
                while ($row_cats=mysqli_fetch_array($run_cats)){
                  
                  $album_name=$row_cats['album_name'];
                  //$cat_title=$row_cats['Cat_title'];
